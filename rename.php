...
if (isset($_FILES['document']) && $_FILES['document']['error'] ==
UPLOAD_ERR_OK) {
...
move_uploaded_file($_FILES['document']['tmp_name'], $upload_dir .
$document_name);
}
// Vérifier les champs obligatoires
if (empty($nom) || empty($prenom) || empty($mail)) {
$message = '<span style="background:red; padding:10px; color:white;
margin:15px;">Veuillez remplir les champs requis</span>';
} else {
...
$message = '<p class="success" style="color:green;">Étudiant mis à jour avec
succès</p>';
...
htmlspecialchars($student['nom']) ?>" required
...
htmlspecialchars($student['prenom']) ?>" required
class="w-full border border-green-300 p-2 rounded focus:outline-none
focus:border-green-500">
</div>
...
htmlspecialchars($student['mail']) ?>" required
class="w-full border border-green-300 p-2 rounded focus:outline-none
focus:border-green-500">
</div>
...
htmlspecialchars($student['adresse']) ?>"
class="w-full border border-green-300 p-2 rounded focus:outline-none
focus:border-green-500">
</div>
...
htmlspecialchars($student['telephone']) ?>"
required class="w-full border border-green-300 p-2 rounded
...
<option value="lycee" <?= ($student['niveau_etude'] == 'lycee') ?
'selected' : '' ?>>Lycée</option>
<option value="universite" <?= ($student['niveau_etude'] ==
'universite') ? 'selected' : '' ?>>Université
</option>
<option value="master" <?= ($student['niveau_etude'] == 'master') ?
'selected' : '' ?>>Master</option>
<option value="doctorat" <?= ($student['niveau_etude'] == 'doctorat') ?
'selected' : '' ?>>Doctorat</option>
</select>
</div>
...
class="w-full border border-green-300 p-2 rounded focus:outline-none
focus:border-green-500"><?= htmlspecialchars($student['interets']) ?></textarea>
</div>
...
alt="Photo de <?= htmlspecialchars($student['nom']) ?>" class="w-16 h16 object-cover rounded mb-2">
<?php endif; ?>
